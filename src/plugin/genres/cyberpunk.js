 ...
 *
 * tokens  — global overrides only (colors, motion, font). Applied via JS
 *           setProperty on html so the genre builder can read and override them.
 *
 * styles  — structural CSS rules compiled into zynaui.css via addBase.
 *           html[data-genre="cyberpunk"] at specificity [0,1,1] beats the ops
 *           defaults on html [0,0,1] so genre activation requires only setting
 *           the data-genre attribute — no JS setProperty needed for structure.
 *           The genre builder's inline setProperty() on html always wins over
 *           these rules when the user customises a token.
 */
...
// Global overrides only — colors, motion character, and font.
// Component structural tokens live in styles['html[data-genre="cyberpunk"]'] below.
export const tokens = {
  // ── Color ───────────────────────────────────────────────────────────────────
  '--zyna':        '#39FF14',
  '--zyna-dark':   '#1A8A00',
