 ...
 *   --btn-icon-size          Square side of a .btn-icon (one line box plus the size's vertical padding, snapped to whole px)
 *
...
export default function(theme) {
  ...
  return {
    ...
    '.btn-primary': {
      ...
      // Reduce visually intense chained filters for motion-sensitive users —
      // a single brightness step gives a subtler, less distracting hover effect.
    ...
    '.btn-sm': {
      ...
      '--btn-icon-size': 'round(calc(1lh + 0.84rem), 1px)',
    },
    ...
    '.btn-lg': {
      ...
      '--btn-icon-size': 'round(calc(1lh + 1.8rem), 1px)',
    },
    ...
    // button at that size in any genre font, snapped to whole pixels so the
    // chamfer edges do not land on sub-pixel boundaries and rasterise
    // differently run to run. --btn-icon-size overrides it.
    '.btn-icon': {
      padding: '0',
      width:  'var(--btn-icon-size, round(calc(1lh + 1.3rem), 1px))',
      height: 'var(--btn-icon-size, round(calc(1lh + 1.3rem), 1px))',
      '--btn-corner': 'var(--z-btn-corner-sm)',
    },
    ...
    '.btn-cut': (({ outer, inner }) => ({ clipPath: outer, borderRadius: '0', '--btn-inner-clip': inner }))(shapes.diagonal('var(--btn-corner)')),
    '.btn-bevel':  (({ outer, inner }) => ({ clipPath: outer, '--btn-inner-clip': inner }))(shapes.bevel('var(--btn-corner)')),
    '.btn-round': { clipPath: shapes.rounded.clipPath, borderRadius: '9999px', '--btn-inner-clip': shapes.rounded.innerClip },
    '.btn-square': { clipPath: shapes.rect.clipPath, borderRadius: shapes.rect.borderRadius, '--btn-inner-clip': shapes.rect.innerClip },
  }
}
