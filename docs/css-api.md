...
| `--btn-icon-size` | — | `calc(1lh + 1.3rem)` | Square side of a `.btn-icon` — one line box plus the size's vertical padding, so it matches the height of a text button beside it. Set automatically by `.btn-sm` / `.btn-lg`. |
