...
- **`.btn-icon` was never square.** It relied on `aspect-ratio: 1`, but the glyph's line box forms an automatic minimum height that the ratio cannot shrink, and the ratio does not widen the box to match it — so an icon button rendered 29×38 in Ops, and a `.btn-sm.btn-icon` came out *taller* than the `.btn-sm` standing next to it. The square side is now one line box plus the same vertical padding the size class uses, snapped to whole pixels (`round(calc(1lh + 1.3rem), 1px)`, `.btn-sm` `round(calc(1lh + 0.84rem), 1px)`, `.btn-lg` `round(calc(1lh + 1.8rem), 1px)`), so an icon button is square and as tall as a text button beside it in any genre font. The new `--btn-icon-size` element-level variable overrides it. Because the side now follows the resolved font, the icon button is no longer part of the `btn-sizes` screenshot — in the two genres whose bundled test font actually loads it could be captured either side of the font swap — and its geometry is asserted numerically across all nine genres instead; visual baselines updated for the button size rows.
### Fixed — charts
...
- **`<zyna-line>` printed every x-axis label on top of its neighbours.** Each labelled point got its label regardless of width, so a series labelled at every point (twelve month names in a 420px card) drew the axis as one unreadable run. Labels are now walked left to right and any whose box would touch the last one kept is hidden; the tick marks stay as point markers and sparse or short labels are untouched.
...
- **`<zyna-orbital>` labels spilled out of the element in narrow containers.** The rose was sized to 80% of the box with no allowance for the labels outside it, and the SVG keeps overflow visible, so at 320px the right-hand labels started 300px in and ran over whatever sat beside the chart. The rose now leaves room for the longest label (plus the leader foot) on each side, down to a 30% floor, and a label that still cannot fit is trimmed with an ellipsis; wide charts keep the full radius.
...
- **`<zyna-waffle>` painted an opaque white square behind its cells in light themes.** The SVG set `background: #FFFFFF` whenever the theme was light, so on Corporate's ivory, Washi's cream, or any light page the grid sat in a white box. The SVG no longer paints a background; the cells sit on the page surface in every theme.
...
- **`<zyna-nightingale>` clipped its side labels in narrow containers.** The label ring sat a fixed 70px beyond the rose, so at 320px the side labels were anchored 7px from the SVG edge and most of a long label was cut off. The ring margin now gives way first (down to 24px), then the rose radius (down to 30% of the half-width), and any label that still cannot fit is trimmed with an ellipsis; wide charts are unchanged.
...
- **The visual job failed on main for a Phosphor badge snapshot whose tree had just passed on the PR.** A macOS runner VM occasionally anti-aliases the 1px badge rims at fractional box edges with different coverage (same commit, same runner image; the diff is only the rim pixels), which the 64px budget cannot absorb. The page scaffold now starts every injected font face loading and lets layout settle for two frames before a screenshot instead of trusting `document.fonts.ready`, which can resolve before the faces have even started loading, and the suite retries a failed screenshot twice on CI only. A genuine regression still fails every attempt; locally the render is deterministic and there are no retries.
